...
def get_logs():
    ...
    c = conn.cursor()
    c.execute("""
        SELECT id_log, date, time, message, type, severity, id_host FROM Log
    """)
    rows = c.fetchall()
    conn.close()
...
def get_user_logs(id_user: int):
    ...
    query = """
        SELECT Log.date, Log.time, Log.message, Log.type, Log.severity, Host.ip_address,Log.id_host
        FROM Log
...
def insert_json(log_json):
    ...
    for data in log_json:
        c.execute("INSERT INTO Log (date,time , message,type,severity,id_host) VALUES (?, ?, ?,?,?,?)",
                  (data.get("date"), data.get("time"), data.get("message"), data.get("category_log"),
                   data.get("severity"),data.get("message"),1,data.get("message"),data.get("message")
                   ))
